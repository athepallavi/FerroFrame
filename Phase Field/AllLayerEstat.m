...
quarter_width = (Lx+ferro_region_limit) / 4;

% Define the x-coordinates for the division into quarters
quarter_coords = [-Lx, -Lx + 2*quarter_width, -Lx + 2*2 * quarter_width, -Lx + 2*3 * quarter_width, -Lx + ferro_region_limit];

% Iterate through each point in the grid
for i = 1:nxf
    for j = 1:nyf
        if Y(i, j) <= ferro_region_limit
            % Check which quarter the point belongs to and assign the polarization
            if Y(i, j) <= quarter_coords(2)
                p(i, j) = ps;  % Positive polarization
            elseif Y(i, j) <= quarter_coords(3)
                p(i, j) = -ps;  % Negative polarization
            elseif Y(i, j) <= quarter_coords(4)
                p(i, j) = ps;  % Positive polarization
            else
                p(i, j) = -ps;  % Negative polarization
            end
